    ...
    const loadAdminData = async () => {
      ...
      try {
        const [dashboardStats, pendingRes, monthly, countersRes] = await Promise.all([
          getDashboardStats(),
          getPendingAuctions(pendingPage, 5),
          getMonthlyCompletedStats(),
          getAdminCounters().catch(() => ({ data: { pendingDisputes: 0 } })),
        ]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Link to="/admin/refund-requests" className="flex items-center gap-2 px-4 py-2 bg-white/60 backdrop-blur-xl border border-slate-200/60 rounded-[1.2rem] text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-rose-500 hover:border-rose-500/20 transition-all shadow-sm">
            <RefreshCw className="w-4 h-4" /> Refunds</Link>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {pendingAuctions.length === 0 ? (
                <div className="bg-white/60 backdrop-blur-xl border border-slate-200/60 rounded-[2rem] p-12 text-center shadow-sm">
                  ...
                  <p className="text-slate-500 font-medium">لا توجد مزادات جديدة بحاجة للمراجعة حالياً.</p>
                </div>
              ) : (
                <>
                    ...
                    {pendingAuctions.map((auction) => (
                      <div key={auction._id} className="bg-white/80 backdrop-blur-xl p-5 md:p-6 rounded-[1.5rem] border border-slate-200/60 flex flex-col md:flex-row md:justify-between items-start md:items-center gap-6 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="flex gap-4 items-center">
                          <div className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center border border-amber-100 flex-shrink-0">
                            <Gavel className="w-6 h-6 text-amber-500" />
                          </div>
                          <div>
                            <h4 className="font-black text-lg text-slate-900 mb-1">{auction.title}</h4>
